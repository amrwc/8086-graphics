...
Animation_Circle_Finish_Repeat:
    xor     cx, cx                      ; Delay
    mov     dx, 0750h
    mov     ah, 86h
    int 15h
...
anim_circle_finish_test2:
    mov     si, word [bp + circ_r]      ; if (r > x || err > y) err += ++x*2+1;
    cmp     si, word [bp - x]
    jg      anim_circle_finish_test2_execute
    mov     si, word [bp - err]
    cmp     si, word [bp - y]
    jg      anim_circle_finish_test2_execute
    jmp     anim_circle_finish_test_exit

anim_circle_finish_test2_execute:
...
;____________________
anim_circle_finish_test_exit:
    cmp     [bp - x], word 0            ; if (x < 0) break;
    jl      Animation_Circle_Finish_Repeat
